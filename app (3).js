...
function buildResponse(t, r){
  ...
  const isBoxed = t.playType === 'boxed';

  // Detect the "digits actually matched, but not eligible for a Box payout" case
  // (this only happens on Boxed plays where every digit is identical).
  const baseDigitsMatch = multisetEqual(t.player, t.winning);
  const baseIneligible = isBoxed && baseDigitsMatch && !r.baseWin;

  let fireballIneligibleCombo = null;
  if(isBoxed && !r.fireballWin){
    for(const c of r.dedupedCombos){
      if(multisetEqual(t.player, c)){ fireballIneligibleCombo = c; break; }
    }
  }

  const paragraphs = [];

  function fireballWinPhrase(){
    if(t.playType === 'straight'){
      const posEntry = r.rawCombos.find(c => c.combo === r.winningCombo);
      const posNote = posEntry ? `swapped into position ${posEntry.position} of the winning number and created ${r.winningCombo}` : `created a matching combination, ${r.winningCombo}`;
      return `the Fireball number ${t.fireball} ${posNote}, exactly what you played`;
    }
    return `the Fireball number ${t.fireball} created a valid combination (${r.winningCombo}) that also matched your numbers as a ${r.fireballCategory}`;
  }

  if(r.baseWin && r.fireballWin){
    paragraphs.push(`Hi! Good news on this one. Your ${draw} ${pt} play was a winner on both sides.`);
    const baseMatchPhrase = t.playType === 'straight'
      ? `matched the winning number ${t.winning} exactly`
      : `matched the winning number ${t.winning} as a ${r.baseCategory}`;
    paragraphs.push(`You played ${t.player}, and it ${baseMatchPhrase}, so your Base wager of ${fmtMoney(r.baseWager)} paid out at ${r.baseOdds} to 1. That's ${fmtMoney(r.baseWinnings)}. On top of that, ${fireballWinPhrase()}, so your ${fmtMoney(r.fireballWager)} Fireball wager paid out too, at ${r.fireballOdds} to 1 for another ${fmtMoney(r.fireballWinnings)}.`);
    paragraphs.push(`Total winnings: ${fmtMoney(r.totalWinnings)}. Congrats!`);

  } else if(r.fireballWin){
    if(baseIneligible){
      paragraphs.push(`Thanks for reaching out! Your ${draw} Boxed play was ${t.player}, matching the winning number ${t.winning} digit for digit. Since Boxed plays need two different digits to qualify for the Box payout, and ${t.player} has identical digits, the Base side isn't eligible even though the numbers line up.`);
    } else {
      paragraphs.push(`Thanks for reaching out! Your ${draw} ${pt} play of ${t.player} didn't match the winning number, ${t.winning}, so the Base side didn't win this time.`);
    }
    paragraphs.push(`Good news, though: ${fireballWinPhrase()}. That means your ${fmtMoney(r.fireballWager)} Fireball wager was a winner at ${r.fireballOdds} to 1, for ${fmtMoney(r.fireballWinnings)}.`);
    paragraphs.push(`Total winnings: ${fmtMoney(r.totalWinnings)}.`);

  } else if(r.baseWin){
    const baseMatchPhrase = t.playType === 'straight'
      ? `matched the winning number ${t.winning} exactly`
      : `matched the winning number ${t.winning} as a ${r.baseCategory}`;
    paragraphs.push(`Thanks for reaching out! Your ${draw} ${pt} play of ${t.player} ${baseMatchPhrase}, so your Base wager was a winner at ${r.baseOdds} to 1, for ${fmtMoney(r.baseWinnings)}.`);
    if(fireballIneligibleCombo){
      paragraphs.push(`The Fireball number ${t.fireball} did create a matching combination (${fireballIneligibleCombo}), but Boxed plays need two different digits to qualify for the Fireball payout, and ${fireballIneligibleCombo} has identical digits, so it isn't eligible this time.`);
    } else {
      paragraphs.push(`The Fireball number ${t.fireball} didn't create a matching combination this time, so the Fireball side didn't win.`);
    }
    paragraphs.push(`Total winnings: ${fmtMoney(r.totalWinnings)}.`);

  } else if(baseIneligible || fireballIneligibleCombo){
    paragraphs.push(`Thanks for checking on this! Your ${draw} Boxed play was ${t.player}, and the winning number was ${t.winning}.`);
    if(baseIneligible && fireballIneligibleCombo){
      paragraphs.push(`I want to explain this one clearly. Your numbers match the winning number digit for digit, and the Fireball number ${t.fireball} also creates a matching combination (${fireballIneligibleCombo}). However, Boxed plays require two different digits to qualify for the Box payout, and since ${t.player} has two identical digits, neither side is eligible, even though the digits line up.`);
    } else if(baseIneligible){
      paragraphs.push(`I want to explain this one clearly. Your numbers match the winning number digit for digit. However, Boxed plays require two different digits to qualify for the Box payout, and since ${t.player} has identical digits, it isn't eligible, even though the numbers line up.`);
    } else {
      paragraphs.push(`I want to explain this one clearly. The Fireball number ${t.fireball} does create a combination that matches your numbers exactly (${fireballIneligibleCombo}). However, Boxed plays require two different digits to qualify for the Box payout, and since ${fireballIneligibleCombo} has identical digits, it isn't eligible for that category, even though the digits line up.`);
    }
    paragraphs.push(`Unfortunately, this play wasn't a winner this time, but thank you for playing, and let us know if you have any other questions!`);

  } else {
    paragraphs.push(`Thanks for reaching out! Unfortunately, your ${draw} ${pt} play of ${t.player} didn't match the winning number, ${t.winning}, and none of the Fireball combinations generated from the number ${t.fireball} lined up with your play either.`);
    paragraphs.push(`This one wasn't a winner, but we appreciate you playing. Good luck on the next one!`);
  }
