...
function boxCategory(len, str, table){
  ...
  // A Boxed number with every digit identical has only one possible arrangement,
  // so it pays at the Straight rate instead of being discarded.
  if(pattern === String(len)){
    return { name: 'Straight', odds: table[len].straight };
  }
  return null;
}
  ...
  <div class="ticket" data-id="${id}">
    ...
    <div class="field-grid">
      ...
      <div class="field col-1">
        ...
        <input type="number" data-field="bet" min="0.01" step="0.01" placeholder="1.00" value="1.00">
...
function ticketIsComplete(t){
  return t.player.length === t.draw && /^\d+$/.test(t.player) &&
         t.winning.length === t.draw && /^\d+$/.test(t.winning) &&
         t.fireball.length === 1 && /^\d$/.test(t.fireball) &&
         t.bet > 0 && !isNaN(t.bet);
}

function clearResultsDisplay(node){
...
function maybeAutoRecalc(node, id){
  ...
  if(state && state.calculated){
    ...
    // Skip silently while the user is mid-edit (e.g. clearing a digit before
    // typing its replacement) instead of flashing a validation error.
    if(!ticketIsComplete(t)) return;
...
function buildResponse(t, r){
  ...
  const playLabel = `${draw} ${pt}`;

  const playerFmt = fmtCombo(t.player);
  const winningFmt = fmtCombo(t.winning);
  const fireballMatchFmt = r.winningCombo ? fmtCombo(r.winningCombo) : null;

  const baseMatchClause = r.baseCategory === 'Straight'
    ? 'exactly'
    : `as a ${r.baseCategory} (any order)`;
  const fireballMatchClause = (r.fireballCategory === 'Straight' || !r.fireballCategory)
    ? ''
    : ` (matching as a ${r.fireballCategory}, any order)`;

  const paragraphs = [];

  if(r.baseWin && r.fireballWin){
    // ---- Base + Fireball Win ----
    paragraphs.push(`Thanks for reaching out!`);
    paragraphs.push(`Great news, you won on both the Base (regular) draw and the Fireball draw!`);
    paragraphs.push(`Your ${playLabel} play of ${playerFmt} matched the official winning number ${winningFmt} ${baseMatchClause}, so your ${fmtMoney(r.baseWager)} Base wager pays ${r.baseOdds} to 1, for a total prize of ${fmtMoney(r.baseWinnings)}.`);
    paragraphs.push(`In addition, using the Fireball number ${t.fireball}, the winning number also becomes ${fireballMatchFmt}, which matches your play${fireballMatchClause}. Your ${fmtMoney(r.fireballWager)} Fireball wager pays ${r.fireballOdds} to 1, for an additional ${fmtMoney(r.fireballWinnings)}.`);
    paragraphs.push(`Total winnings: ${fmtMoney(r.totalWinnings)}.`);
    paragraphs.push(`Congratulations, and thanks for playing!`);

  } else if(r.fireballWin){
    // ---- Fireball Win Only ----
    paragraphs.push(`Thanks for reaching out!`);
    paragraphs.push(`Good news, you won with Fireball!`);
    paragraphs.push(`Your original play of ${playerFmt} did not match the official winning number ${winningFmt}, so the Base (regular) wager was not a winner.`);
    paragraphs.push(`However, using the Fireball number ${t.fireball}, one possible winning combination becomes ${fireballMatchFmt}, which matches your play${fireballMatchClause}.`);
    paragraphs.push(`Your ${fmtMoney(r.fireballWager)} Fireball wager pays ${r.fireballOdds} to 1, for a total prize of ${fmtMoney(r.fireballWinnings)}.`);
    paragraphs.push(`Reminder: When you add Fireball, your total wager is split evenly between the Base (regular) draw and the Fireball draw.`);
    paragraphs.push(`Congratulations!`);

  } else if(r.baseWin){
    // ---- Base Win Only ----
    paragraphs.push(`Thanks for reaching out!`);
    paragraphs.push(`Great news, you won on the Base (regular) draw!`);
    paragraphs.push(`Your ${playLabel} play of ${playerFmt} matched the official winning number ${winningFmt} ${baseMatchClause}.`);
    paragraphs.push(`Your ${fmtMoney(r.baseWager)} Base wager pays ${r.baseOdds} to 1, for a total prize of ${fmtMoney(r.baseWinnings)}.`);
    paragraphs.push(`The Fireball number ${t.fireball} did not create an additional matching combination, so the Fireball portion of your ticket was not a winner.`);
    paragraphs.push(`Reminder: When you add Fireball, your total wager is split evenly between the Base (regular) draw and the Fireball draw.`);
    paragraphs.push(`Congratulations!`);
    ...
    // ---- No Winnings ----
    paragraphs.push(`Thanks for reaching out!`);
    paragraphs.push(`Unfortunately, the ticket in question wasn't a winner.`);
    paragraphs.push(`Your ${playLabel} play of ${playerFmt} did not match the official winning number ${winningFmt}. The Fireball number ${t.fireball} also did not create a matching combination, since Fireball must replace one digit of the official winning number to create a possible winning combination matching what you selected.`);
    paragraphs.push(`This ticket did not have any winnings, but we appreciate you playing. Best of luck on the next draw!`);
  }
