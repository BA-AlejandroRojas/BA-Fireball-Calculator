...
function wireDigitGroup(container, node, fieldName){
  ...
  boxes.forEach((box, i) => {
    box.addEventListener('input', () => {
      ...
      if(digit && boxes[i+1]) boxes[i+1].focus();
    ...
    box.addEventListener('paste', (e) => {
      ...
      const id = node.getAttribute('data-id');
      maybeAutoRecalc(node, id);
      const nextEmpty = boxes[pasted.length] || boxes[boxes.length-1];
  ...
  box.innerHTML = `
    ...
    <div class="cards-row">
      <div class="rcard">
        ...
        <div class="rline">${rk('coin','Base Winnings')}<span class="v gold">${fmtMoney(r.baseWinnings)}</span></div>
      </div>
      <div class="rcard fireball-card">
        ...
        <div class="rline">${rk('coin','Fireball Winnings')}<span class="v gold">${fmtMoney(r.fireballWinnings)}</span></div>
